...
@app.route('/')
def home(): 
    return "✅ بوت ختمة الاحترافي (Starter Plan) يعمل بنشاط 24/7"
...
ROLE_NAME = "ختمة"
last_sent_minute = "" # متغير لمنع التكرار في نفس الدقيقة
...
def find_image(number):
    ...
    for filename in os.listdir(image_folder):
        if any(int(n) == number for n in re.findall(r'\d+', filename)):
...
class QuranControlView(View):
    ...
    async def subscribe(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            await interaction.response.send_message(f"✅ تم منحك رتبة {ROLE_NAME} لتصلك التنبيهات!", ephemeral=True)
        except:
            await interaction.response.send_message("❌ فشل منح الرتبة. تأكد من رفع رتبة البوت في الإعدادات.", ephemeral=True)
...
async def إعدادات(ctx):
    ...
    embed.description = "1. اختر القناة المخصصة للورد.\n2. اضغط زر التفعيل للتنبيهات.\n3. استخدم زر التجربة للتأكد."
...
async def تعديل(ctx, num: int):
    ...
    await ctx.send(f"✅ تم تعديل الورد يدوياً. الصفحة القادمة ستكون: **{num}**")

@bot.command()
@commands.has_permissions(administrator=True)
async def تصفير(ctx):
    save_page(4)
    await ctx.send("🔄 تم إعادة ضبط الورد لجميع السيرفرات (الصفحة 4).")
